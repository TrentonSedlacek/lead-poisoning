...
server <- function(input, output, session) {
  ...
  do_fit_model <- function() {
    ...
    # Matches 02.11 behavior: addresses with no other children get outcome=NA
    # and are dropped (not coded as 0). Outcome window uses all children up to
    # the temporal boundary, not restricted to post-feature-child tests.
    ...
    if (input$train_all) {
      ...
      d_keys <- d |> select(AddressIdentifier, first_id) |>
        distinct(AddressIdentifier, .keep_all = TRUE)
