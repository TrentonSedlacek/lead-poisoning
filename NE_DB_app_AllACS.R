           ...
           An XGBoost comparison is always run alongside for benchmarking."),
              fluidRow(
                box(width = 4, title = "Model Settings",
                    ...
                    hr(),
                    checkboxInput("use_ym_census", "Use year-matched ACS", TRUE),
                    p(em("When checked, each address uses the ACS vintage closest to its first test year.
                         When unchecked, all addresses use the 2023 ACS snapshot."),
...
server <- function(input, output, session) {
  ...
  do_fit_model <- function() {
    ...
    use_re <- re != "none"
    use_ym <- isTRUE(input$use_ym_census)
    md <- if (use_ym) {
      if (re == "bg_geoid") model_data_bg_ym else model_data_tract_ym
    } else {
    ...
    tryCatch({
      ...
      if (!is.null(all_addr) && nrow(all_addr) > 0) {
        ...
        if (nrow(single_addr) > 0) {
          ...
          census_src <- if (use_ym) {
            # Year-matched: join by geoid + acs_year
            ts_src <- if (re == "bg_geoid") bg_census_ts else tract_census_ts
            if (!is.null(ts_src)) {
              single_addr_ym <- single_addr |>
                mutate(acs_year = acs_year_map(first_year))
              single_addr_ym |>
                left_join(ts_src, by = c(census_join_col, "acs_year")) |>
                select(-acs_year)
            } else NULL
          } else {
            if (re == "bg_geoid") bg_census else tract_census
          }
          single_df <- if (use_ym && !is.null(census_src)) {
            census_src  # already joined above
          } else if (!is.null(census_src)) {
  ...
  output$dl_sample_table <- downloadHandler(
    ...
    content = function(file) {
      md <- if (isTRUE(input$use_ym_census)) {
        if (input$random_effect == "bg_geoid") model_data_bg_ym else model_data_tract_ym
      } else {
        if (input$random_effect == "bg_geoid") model_data_bg else model_data_tract
      }
