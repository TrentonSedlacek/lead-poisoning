...
format_table <- function(df, id_name, unit = "Children", show_county = FALSE,
  ...
  if ("n_acs_u6" %in% names(df)) {
    out$`ACS Under-6 Pop` <- df$n_acs_u6
    out$`Elevated/1k` <- round(df$rate_per_1k, 1)
    out$`Tested/1k` <- round(df$tested_rate_per_1k, 1)
  }
...
# A10/A11: Vectorized Wilson CI computation
compute_wilson_ci <- function(n_t, n_e, z = 1.96) {
  p_hat <- n_e / n_t
  denom <- 1 + z^2 / n_t
  center <- (p_hat + z^2 / (2 * n_t)) / denom
  margin <- z * sqrt((p_hat * (1 - p_hat) + z^2 / (4 * n_t)) / n_t) / denom
  list(lo = pmax(0, (center - margin)) * 100,
       hi = pmin(1, (center + margin)) * 100)
}
...
join_census_for_mode <- function(df, acs_mode, geo_col, census_ts) {
              ...
              fluidRow(
                valueBox({
                  if (!is.null(acs_u6_ts) && !is.null(acs_u6_ts$state)) {
                    ...
                    if (length(pop_2023) > 0 && pop_2023 > 0) {
                      rate <- round(sum(first_test$elevated, na.rm = TRUE) / pop_2023 * 1000, 1)
                      paste0(rate, " per 1,000")
                    } else "N/A"
                  } else "N/A"
                }, "Cumulative Elevated Rate (per 1,000 Under-6, 2023 ACS)",
                icon = icon("chart-line"), color = "purple", width = 6),
                valueBox({
                  if (!is.null(acs_u6_ts) && !is.null(acs_u6_ts$state)) {
                    pop_2023 <- acs_u6_ts$state$n_children_u6[acs_u6_ts$state$acs_year == 2023]
                    if (length(pop_2023) > 0 && pop_2023 > 0) {
                      format(pop_2023, big.mark = ",")
                    } else "N/A"
                  } else "N/A"
                ...
                box(width = 4, title = "Model Settings",
                    ...
                    radioButtons("acs_mode", "ACS Census Linkage:",
                                 c("Year-matched (each address → closest ACS)" = "ym",
                                   ...
                                   "Categorical year (ACS year as factor)" = "cat_year"),
                    ...
                    p(em("Year-matched: each address uses its closest ACS vintage (may violate IID due to overlapping 5-year windows). ",
                         ...
                         "Categorical year: uses year-matched ACS plus a factor variable for ACS vintage."),
                      style = "font-size: 11px; color: #666;"),
                    hr(),
                    ...
                    p(em("Adds a neighborhood-specific random intercept (compound symmetry correlation structure within groups). ",
                         "Tracts are larger (more stable); block groups are smaller (more local). 'None' runs standard logistic regression."),
                      style = "font-size: 11px; color: #666;"),
                    hr(),
                ...
                box(width = 8, title = "Results",
                   ...
                   removing one variable if correlation > 0.70.", style = "font-size: 12px;"),
                      ...
                      tabPanel("Model Comparison", br(),
                               ...
                               hr(),
                               h5("Mean BLL per Address by Model Screening Order"),
                               p("Average first-child BLL as addresses are screened highest-risk-first. Lower = model is prioritizing higher-BLL addresses.",
...
server <- function(input, output, session) {
  ...
  get_stats <- reactive({
    ...
    if (active_geo %in% c("Tract", "Block Group")) {
      g_col <- if (active_geo == "Tract") "tract_geoid" else "bg_geoid"
      result[[tolower(gsub(" ", "", active_geo))]] <- make_stats(g_col)
    }
    # Backfill keys the rest of the code expects
    if (is.null(result$tract))         result$tract <- NULL
    if (is.null(result$bg))            result$bg    <- NULL
    if (is.null(result$blockgroup))    result$bg    <- result$blockgroup
    result
  })
  ...
  attach_rates <- function(stat_data, geo_level, yr_range) {
    ...
    if (is.null(u6_src)) return(stat_data)
    # Sum ACS population across selected years
    acs_years <- acs_year_map(yr_range[1]):acs_year_map(yr_range[2])
    u6_agg <- u6_src |>
      filter(acs_year %in% acs_years) |>
  ...
  output$trend_summary_table <- renderDT({
    ...
    # For rate metrics, we need to handle display differently
    ...
    yr_rows <- lapply(years, function(yr) {
      ...
      # Attach rates for geo-level stats
      geo_stats <- attach_rates(geo_stats, input$geo_level, c(yr, yr))
      ...
      # For rate metrics, compute state-level rate
  ...
  observeEvent(c(input$geo_level, input$fill_var, input$unit_type, input$year_filter,
    ...
    if (isTRUE(input$show_map_ci)) {
      ...
      if (any(valid)) {
        wilson <- compute_wilson_ci(n_t[valid], n_e[valid])
        ci_pct_lo[valid] <- wilson$lo
        ci_pct_hi[valid] <- wilson$hi
        bll_valid <- valid & !is.na(bll) & n_t >= 2
        if (any(bll_valid)) {
          # Use state-level CV as prior; area CV unknown without raw data per area
          state_cv <- sd(dat$result, na.rm = TRUE) / mean(dat$result, na.rm = TRUE)
    ...
    # Rate tooltip additions
    rate_tip <- ""
    if (var %in% c("rate_per_1k", "tested_rate_per_1k") && "n_acs_u6" %in% names(geo)) {
      rate_tip <- paste0(
        "<br>ACS Under-6 Pop: ", ifelse(!is.na(geo$n_acs_u6), format(geo$n_acs_u6, big.mark = ","), "N/A"),
        "<br>Elev/1k: ", ifelse(!is.na(geo$rate_per_1k), sprintf("%.1f", geo$rate_per_1k), "N/A"),
        "<br>Tested/1k: ", ifelse(!is.na(geo$tested_rate_per_1k), sprintf("%.1f", geo$tested_rate_per_1k), "N/A"))
    }
    if (unit == "Children") {
  ...
  do_fit_model <- function() {
    ...
    if (acs_mode == "endpoint") {
      ...
      test_end_acs  <- acs_year_map(max(years))  # end of available data
    ...
    factor_covars  <- intersect(all_covars, c("income_cat", "first_sex", "first_sample_type", "acs_year"))
    ...
    # For all-addresses mode: compute outcome at runtime based on train/test split
    # Outcome = any child tested at address in TEST period is elevated
    # Training addresses get outcome from test-period children; single-child addresses
    # that have no test-period children get outcome = 0 (no evidence of future elevation)
    train_yrs <- input$train_years
    if (use_all_addr && !input$train_all) {
      # Exclude the feature child (first_id) from outcome to prevent leakage.
      # Without this, the feature child's BLL appears as both predictor and outcome.
      test_period_outcomes <- first_test |>
        filter(sample_year > train_yrs[2]) |>
        inner_join(d |> select(AddressIdentifier, first_id), by = "AddressIdentifier") |>
        filter(PATIENT_LOCAL_ID != first_id) |>
        summarize(test_elevated = as.integer(any(elevated == 1, na.rm = TRUE)),
                  .by = AddressIdentifier)
      d <- d |>
        left_join(test_period_outcomes, by = "AddressIdentifier") |>
        mutate(outcome = coalesce(test_elevated, 0L)) |>
        select(-test_elevated)
    } else if (use_all_addr && input$train_all) {
      # Train-all mode for all addresses: no held-out test period.
      # Use "any other child elevated" as outcome; single-child = 0
      other_child_outcomes <- first_test |>
        inner_join(d |> select(AddressIdentifier, first_id), by = "AddressIdentifier") |>
        filter(PATIENT_LOCAL_ID != first_id) |>
        summarize(other_elevated = as.integer(any(elevated == 1, na.rm = TRUE)),
                  .by = AddressIdentifier)
      d <- d |>
        left_join(other_child_outcomes, by = "AddressIdentifier") |>
        mutate(outcome = coalesce(other_elevated, 0L)) |>
        select(-other_elevated)
    }
  ...
  # Mean BLL per address as screened by model
  output$mean_bll_screened_plot <- renderPlotly({
    test <- rv$test_preds
    if (is.null(test) || nrow(test) == 0 || !"first_bll" %in% names(test)) return(NULL)
    n <- nrow(test)
    glmer_ord <- order(test$pred, decreasing = TRUE)
    cum_bll_glmer <- cumsum(test$first_bll[glmer_ord]) / seq_len(n)
    idx <- unique(c(1, round(seq(1, n, length.out = 300)), n))
    pct_scr <- idx / n * 100
    p <- plot_ly() %>%
      add_trace(x = pct_scr, y = cum_bll_glmer[idx], type = "scatter", mode = "lines",
                name = "GLMER", line = list(color = "#2c7fb8", width = 2.5))
    if (!is.null(rv$xgb_test_preds)) {
      xgb_ord <- order(rv$xgb_test_preds, decreasing = TRUE)
      cum_bll_xgb <- cumsum(test$first_bll[xgb_ord]) / seq_len(n)
      p <- p %>% add_trace(x = pct_scr, y = cum_bll_xgb[idx], type = "scatter", mode = "lines",
                            name = "XGBoost", line = list(color = "#d73027", width = 2.5))
    }
    overall_mean <- mean(test$first_bll, na.rm = TRUE)
    p %>% add_trace(x = c(0, 100), y = c(overall_mean, overall_mean), type = "scatter", mode = "lines",
                     name = "Overall Mean", line = list(color = "gray60", width = 1, dash = "dash")) %>%
      layout(title = list(text = "Mean BLL per Address Screened", font = list(size = 13)),
             xaxis = list(title = "% Addresses Screened", ticksuffix = "%"),
             yaxis = list(title = "Cumulative Mean BLL (\u00b5g/dL)"),
             legend = list(orientation = "h", y = -0.2), margin = list(t = 50))
  })
